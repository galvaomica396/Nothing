...
- **탐지는 보조, 최종 결정은 사용자입니다.** Nothing은 개인정보 후보를 탐지·경고하고
  자동으로 가리지만, 어떤 자동 탐지기도 누락·오탐이 있을 수 있습니다. 최종 저장 전
  화면에서 직접 확인하고, 놓친 항목은 **수동 드래그 마스킹**으로 가리고, 잘못 가려진
  항목은 복원하십시오.
- **미해결 경고가 있어도 저장은 사용자의 선택으로 진행됩니다.** 이때 무엇이 가려지지
  않았는지 저장 시점에 고지되며 리포트에 기록됩니다. 산출물을 외부에 공유하기 전
  최종 확인 책임은 사용자에게 있습니다.
- **텍스트 레이어가 없는 순수 스캔 PDF는 자동 마스킹이 지원되지 않습니다.** 이 경우
  앱이 검토 필요 안내를 표시합니다. (OCR 엔진 통합은 후속 버전 예정)
- 본 소프트웨어는 상용 제품이 아니며, AGPL-3.0 하에 **어떠한 보증 없이** 제공됩니다.
