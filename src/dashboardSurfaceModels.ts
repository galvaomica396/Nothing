...
const REVIEW_REASON_LABELS: Readonly<Record<string, string>> = {
  ...
  manual_action_carry_requires_review: "이전 수동 가림 위치를 다시 확인해야 함",
  reanalysis_decision_carried: "이전 사용자 검토 결정을 유지함",
  reanalysis_decision_not_carried: "새 검출과 이전 결정이 일치하지 않아 다시 확인해야 함",
  reanalysis_exclude_superseded_by_custom_keyword: "새 사용자 키워드 가림이 이전 제외 결정보다 우선함",
  reanalysis_exclude_superseded_by_manual_mask: "수동 가림이 이전 제외 결정보다 우선함",
  reanalysis_occurrence_match_ambiguous: "여러 검출과 겹쳐 이전 결정을 자동 승계하지 않음",
  reanalysis_occurrence_match_missing: "대응 검출이 없어 이전 결정을 자동 승계하지 않음",
